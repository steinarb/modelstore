...
class ModelContextRecordingMetadataTest {
    ...
    void testCreateModelContextRecordingMetadata() {
        var outbackId = UUID.fromString("5b66f36b-4de8-4099-9044-ad9fcc6dc4d1");
        var unwrappedOutback = nonMetadataRecordingContext.findPropertyset(outbackId);
        var wrappedOutback = context.findPropertyset(outbackId);
    ...
    void testInitialPropertysetModifiedTimeValuesModifyPropertysetsSaveAndRestor() throws IOException {
        ...
        var outbackId = UUID.fromString("5b66f36b-4de8-4099-9044-ad9fcc6dc4d1");
        var outback1 = context.findPropertyset(outbackId);
        assertNull(context.getLastmodifieddate(outback1));
        var tacomaId = UUID.fromString("7fd99b6c-603c-4965-8fc3-00e2d2ac2202");
        var tacoma1 = context.findPropertyset(tacomaId);
        assertNull(context.getLastmodifieddate(tacoma1));
        var volvoId = UUID.fromString("31c4b2e9-c66f-40d6-9b5e-fe8a26947460");
        var volvo1 = context.findPropertyset(volvoId);
        assertNull(context.getLastmodifieddate(volvo1));
        var nakamuraId = UUID.fromString("384d417e-a38a-4b5d-ad39-23148545a1b8");
        Propertyset nakamura1 = context.findPropertyset(nakamuraId);
        assertNull(context.getLastmodifieddate(nakamura1));
        var ferrariId = UUID.fromString("bb87fff5-9582-4ab1-9890-942fc35d44b2");
        var ferrari1 = context.findPropertyset(ferrariId);
    ...
    private Propertyset createMetadataWithErrors(ModelContext context) {
        var metadataWithErrors = context.createPropertyset();
        var lastmodifiedtimes = context.createPropertyset();
    ...
    void testEquals() {
        ...
        var identicalInner = new ModelContextImpl(modelstore);
        addPropertysetsToContext(identicalInner);
        var identicalContext = new ModelContextRecordingMetadata(identicalInner, dateFactory, valueCreator);
